...
void CameraWebServer_AP::CameraWebServer_AP_Init(void)
{
  ...
  char* wifiSSIDs[]={"Pixel_9815","Galaxy A713671","WLAN_UUFRJ"};
  char* wifiPasswords[]={"8u5r86awdiztzcu","dvyg7269","HenterscheBaeffzgaGel#"};
  int wifiIndex=0;
  WiFi.begin(wifiSSIDs[wifiIndex%3], wifiPasswords[wifiIndex++%3]);
  Serial.print("Connecting to WiFi ");
  Serial.println(wifiSSIDs[(wifiIndex-1)%3]);
  bool run = true;
  ...
  while (run)
  {
    ...
    case WL_CONNECTION_LOST:
      Serial.print("Connecting to WiFi ");
      Serial.println(wifiSSIDs[(wifiIndex)%3]);
      WiFi.begin(wifiSSIDs[wifiIndex%3], wifiPasswords[wifiIndex++%3]);
      delay(1000);
    case WL_CONNECTED:
